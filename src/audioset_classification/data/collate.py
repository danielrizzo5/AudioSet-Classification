...
def collate_clap_inputs(
    batch: list[tuple[torch.Tensor, torch.Tensor]],
) -> tuple[torch.Tensor, torch.Tensor]:
    """Pad ``input_features`` along time; stack ``is_longer`` (no labels).

    Args:
        batch: List of (input_features [C, T, F], is_longer [1, 1]).

    Returns:
        input_features [B, C, T_max, F], is_longer [B, 1].
    """
    feats = [b[0] for b in batch]
    longers = [b[1] for b in batch]
    ...
    if batched_longers.dim() == 3:
        batched_longers = batched_longers.squeeze(1)
    return batched_feats, batched_longers
